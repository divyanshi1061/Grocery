...
import { BarChart as BarChartIcon, IndianRupee, Package, Truck, User } from 'lucide-react'
...
function AdminDashboardClient({ earning, stats,chartData }: propType) {
    ...
    return (
        <div className='pt-28 w-[90%] ms:w-[80%] mx-auto'>
            ...
            <motion.div initial={{ opacity: 0, y: 15 }}
                ...
                className="bg-green-50 border border-gereen-200 shadow-sm rounded-2xl p-6 text-center mb-10">
                <h2 className='text-4xl font-extrabold text-orange-700 mb-2'>{title}</h2>
                <p className='text-4xl font-extrabold text-orange-800'>₹{(currentEarning || 0).toLocaleString()}</p>
            </motion.div>
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10'>
                {stats.map((s, i) => {
                    const icons = [
                        <Package key="p" className='text-orange-700 w-6 h-6' />,
                        <User key="u" className='text-orange-700 w-6 h-6' />,
                        <Truck key="t" className='text-orange-700 w-6 h-6' />,
                        <IndianRupee key="r" className='text-orange-700 w-6 h-6' />
                    ]
                    return <motion.div
                        key={i}
                        initial={{ opacity: 0, y: 20 }}
